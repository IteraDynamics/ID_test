"""Run the bounded development comparison described in ML_BREADTH_TRAINING_20260910.md."""
...
from scripts.run_ml_etf_risk_screen import features, weekly_sessions
...
def stopping_split(train):
    dates=np.sort(train.date.unique())
    if len(dates)<310:raise ValueError('Insufficient stopping history')
    boundary=dates[-52]
    sub=train.loc[train.label_end<boundary]
    stop=train.loc[(train.date>=boundary)&train.weekly]
    if sub.date.nunique()<252 or stop.empty:raise ValueError('Stopping support')
...
def fit(name,train,val):
    ...
    extra={'optimizer_fits':1};curve=[]
    with threadpool_limits(limits=1),warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter('always')
        model=estimator(name)
        if name.startswith(('mlp','sequence')):
            sub,stop=stopping_split(train)
            scaler=StandardScaler().fit(sub[columns]);x=scaler.transform(sub[columns]);y=100*sub.target.to_numpy()
            sx=scaler.transform(stop[columns]);sy=100*stop.target.to_numpy()
            net=clone(model.steps[-1][1]).set_params(shuffle=True)
            best=float('inf');best_epoch=0
            for epoch in range(1,201):
                net.partial_fit(x,y)
                if epoch%5:continue
                loss=float(np.mean((net.predict(sx)-sy)**2))/10000
                curve.append({'epoch':epoch,'training_mse':float(np.mean((net.predict(x)-y)**2))/10000,'stopping_mse':loss})
                if loss<best-1e-10:best=loss;best_epoch=epoch
                if epoch-best_epoch>=25:break
            if not np.isfinite(best):raise ValueError('Nonfinite stopping loss')
            scaler=StandardScaler().fit(train[columns]);x=scaler.transform(train[columns]);y=100*train.target.to_numpy()
            net=clone(model.steps[-1][1]).set_params(shuffle=True)
            for _ in range(best_epoch):net.partial_fit(x,y)
            prediction=net.predict(scaler.transform(val[columns]))/100
            fitted=net.predict(x)/100
            extra.update(optimizer_fits=2,selected_epoch=best_epoch,epochs_examined=epoch,hit_epoch_cap=epoch==200,
                stopping_fit_last_outcome=str(sub.label_end.max()),stopping_first_signal=str(stop.date.min()),
                stopping_last_outcome=str(stop.label_end.max()),training_curve=json.dumps(curve))
        else:
            model.fit(train[columns],100*train.target)
            prediction=model.predict(val[columns])/100
            fitted=model.predict(train[columns])/100
    if not np.isfinite(prediction).all() or not np.isfinite(fitted).all(): raise ValueError('Nonfinite prediction')
    return prediction,{'training_mse':float(np.mean((fitted-train.target.to_numpy())**2)),
      ...
      'warnings':' | '.join(str(w.message) for w in caught),**extra}
...
def run(root,output,smoke=False):
    ...
    for year in years:
        ...
        pd.concat(predictions).to_csv(output/'checkpoint_forecasts.csv',index=False)
        pd.DataFrame(scores).to_csv(output/'checkpoint_fits.csv',index=False)
    ...
    report={'status':'DEVELOPMENT_DIAGNOSTIC_ONLY','smoke':smoke,'fits':sum(s['optimizer_fits'] for s in scores),'candidate_evaluations':len(scores),'sources':sources,
      ...
      'spec_sha256':hashlib.sha256((Path(__file__).resolve().parents[2]/'docs/research/ML_BREADTH_TRAINING_20260910.md').read_bytes()).hexdigest(),
      'input_loader_sha256':hashlib.sha256(Path(__file__).with_name('breadth_inputs.py').read_bytes()).hexdigest(),
