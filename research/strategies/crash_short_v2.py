"""Layer 2 — CrashShortV2 (Crash-Only / Macro-Bear Hedge Sleeve — v2).

Philosophy
----------
Incremental iteration on crash_short_v1. Two surgical changes only.

What v1 taught us
-----------------
The ATR cap (6.0%) was the primary binding constraint in 2022. During
deep crash phases, BTC's hourly ATR% regularly exceeded 6%, causing the
strategy to reject genuine crash-regime entries as "fill quality too poor".
Lowering the drawdown gate (25% → 20%) had zero effect because by the time
all other conditions aligned, price was already far past both thresholds.

Changes vs v1
-------------
1. ENTRY_ATR_CAP raised from 6.0% → 7.0%.
   The 6% ceiling blocked entries during the most active crash hours.
   7% still provides a meaningful buffer below the 8% crisis level.

2. ATR_PERSIST_BARS = 3 (new).
   Requires ATR to remain inside [MIN_ATR_PCT, ENTRY_ATR_CAP] for
   ATR_PERSIST_BARS consecutive bars before entry is allowed.
   Bull corrections produce a spike that normalises within 1–2 bars;
   a 3-bar sustained ATR window is characteristic of established crash
   volatility, not a short-lived correction spike.

Everything else — exits, sizing, macro EMA, drawdown gate, EMA spread
persistence, spread momentum — is unchanged from v1.
...
Entry gate — ALL must hold:
    ...
    5. ATR% in [2.5%, 7.0%] for ATR_PERSIST_BARS consecutive bars
       (crash is live, fill quality acceptable, not a transient spike).
    6. spread_momentum < 0 (trend still accelerating bearishly at entry).
...
ENTRY_ATR_CAP = 0.070           # 7.0% ceiling — raised from v1's 6.0%
ATR_PERSIST_BARS = 3            # ATR must stay in window for this many bars
...
def generate_intent(
    ...
    # Gate 2: crisis ATR — reject before any other check
    if atr_pct > CRISIS_ATR_PCT:
