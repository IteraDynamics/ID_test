"""Layer 2 — TrendFollowingV8 (Regime-Graduated Entry Sizing).
...
The problem: entries during HIGH_VOL regime (ATR > 4%) combined with 90%
...
v8 design: two-tier regime-graduated entry
------------------------------------------
Instead of all-or-nothing (TREND_UP only vs all regimes), size is adjusted
based on the risk profile of each regime:
...
Tier 2 — TREND_UP, VOL_COMPRESSION: **90% exposure, spread > 0.4%**
  Well-characterised, moderate-vol trend conditions.  Full allocation.

Tier 3 — RANGE, VOL_EXPANSION: **70% exposure, spread > 0.8%**
  Trend exists but conditions are ambiguous or vol is expanding.
  Reduced allocation captures the upside while limiting crash exposure.
  Higher spread threshold (0.8% vs 0.4%) requires a more established trend.

Additional changes from v7
---------------------------
- CRISIS_ATR_PCT: 5.5% → 5.0% — tighter crisis exit to reduce flash-crash
  losses; still above the 4% HIGH_VOL regime threshold to maintain hysteresis.
- Add-on (90% → 100%): only fires in TREND_UP regime, not in RANGE/VOL_EXPANSION.
"""
...
BASE_EXPOSURE = 0.90
MIN_ENTRY_SPREAD = 0.004        # 0.4% EMA spread

# ── Tier 2 exposure (RANGE / VOL_EXPANSION) ───────────────────────────────────
ELEVATED_EXPOSURE = 0.70
ELEVATED_ENTRY_SPREAD = 0.008   # 0.8% EMA spread — stronger signal required
...
def generate_intent(
    ...
    if already_long:
        ...
        # Add-on: only in confirmed TREND_UP — not in ambiguous regimes
        if at_base and ctx.regime == RegimeLabel.TREND_UP:
            strong_trend = (
    ...
    if bullish_entry:
        return StrategyIntent(
            ...
            reason=(
                ...
                f"spread={ema_spread:.3f} → {entry_exposure:.0%} exposure"
            ),
            meta=meta,
            strategy_id=STRATEGY_ID,
        )
    ...
    return StrategyIntent(
        ...
        desired_exposure_frac=0.0,
        horizon_hours=0,
        reason=f"No signal: spread={ema_spread:.4f} required={required_spread} ({tier})",
        meta=meta,
        strategy_id=STRATEGY_ID,
    )
