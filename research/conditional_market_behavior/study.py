...
OUTCOMES=('log_return','log_variance','downside_semivariance','max_adverse_excursion','max_favorable_excursion','absolute_return','path_efficiency','sign_persistence');STAB_COLS=PCS+TRAJECTORY+['episode_age']
def add_forward_outcomes(panel,hourly):
 grid=hourly.reindex(pd.date_range(hourly.index[0],hourly.index[-1],freq='h'));close=grid.close.copy();close.index+=HOUR;daily=close.reindex(panel.index);out=panel.copy()
 for h in HORIZONS:
  rs=pd.concat([np.log(daily.shift(-k)/daily.shift(-(k-1))) for k in range(1,h+1)],axis=1);rel=pd.concat([daily.shift(-k)/daily-1 for k in range(1,h+1)],axis=1);complete=rs.notna().all(axis=1);endpoint=rs.sum(axis=1,min_count=h);path=rs.abs().sum(axis=1,min_count=h);same=((np.sign(rs).eq(np.sign(endpoint),axis=0))&rs.ne(0)).sum(axis=1);nonzero=rs.ne(0).sum(axis=1)
...
def _ridge(train,test,target,cols):
 ...
 if cols:transformers.append(('x',StandardScaler(),cols))
 pre=ColumnTransformer(transformers);m=make_pipeline(pre,Ridge(alpha=10.));m.fit(tr[['core_label',*cols]],tr[target]);return te.index,m.predict(te[['core_label',*cols]])
def evaluate(hourly,asset,hours,years=range(2020,2026)):
 ...
 for year in years:
  cutoff=pd.Timestamp(f'{year}-01-01',tz='UTC');finish=pd.Timestamp(f'{year+1}-01-01',tz='UTC');fit_base=base.loc[(base.index<cutoff)&(base.label_end<cutoff)];eval_base=base.loc[(base.index>=cutoff)&(base.index<finish)];tm=TrajectoryModel().fit(fit_base);train=tm.transform(fit_base);test=tm.transform(eval_base);train['episode_age']=all_age.reindex(train.index);test['episode_age']=all_age.reindex(test.index);sm=_stability_model(train);test['instability_7d']=_predict_stability(sm,test,test.episode_age);train['instability_7d']=np.nan
  ...
  for h in HORIZONS:
   for target in OUTCOMES:
    col=f'{target}_{h}d';tr=train.loc[train[f'outcome_end_{h}d']<cutoff].copy();te=test.copy();reps=[('core',[]),('core_plus_pca4',PCS),('core_plus_instability',['instability_7d']),('core_plus_pca4_plus_instability',PCS+['instability_7d'])];pred={}
    for name,cols in reps:
