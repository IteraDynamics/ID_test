...
def main():
 p=argparse.ArgumentParser();p.add_argument('--data-root',type=Path,required=True);p.add_argument('--output-root',type=Path,default=Path('artifacts'));a=p.parse_args();out=a.output_root/('latent_transition_geometry_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S_%fZ'));out.mkdir(parents=True,exist_ok=False);error=None
 try:
  ...
  root=Path(__file__).resolve().parents[2];report=dict(status='observation-only latent transition geometry hypothesis test',source_lock=lock,sources=sources,horizons=list(HORIZONS),sampling='UTC midnight; yearly 2020-2025 walk-forward; training-only fits',uncertainty='No trading/runtime/promotion claim',versions=dict(python=platform.python_version(),pandas=pd.__version__,numpy=np.__version__,sklearn=sklearn.__version__),git_commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),study_sha256=hashlib.sha256(Path(__file__).with_name('study.py').read_bytes().replace(b'\r\n',b'\n')).hexdigest());(out/'report.json').write_text(json.dumps(report,indent=2)+'\n');print('Complete: review destination scores, pair support, and lead geometry together.')
 except Exception:
