#!/usr/bin/env python
"""Itera Dynamics — Long Volatility Sleeve v1 Backtest Runner.

Runs vol_long_v1 against a long-vol instrument/proxy such as VIXY or VXX.

Outputs:
    artifacts/vol_long_v1/
        equity_curve.csv
        trades.csv
        summary.json
"""
...
from research.strategies.vol_long_v1 import STRATEGY_ID, generate_intent
...
def main() -> None:
    p = argparse.ArgumentParser(description="Run Long Volatility Sleeve v1 backtest")
    p.add_argument("--data", required=True)
    p.add_argument("--asset", default="VIXY")
    p.add_argument("--start", default=None)
    p.add_argument("--end", default=None)
    p.add_argument("--capital", type=float, default=100000)
    p.add_argument("--out-dir", default="artifacts/vol_long_v1")
    args = p.parse_args()
    ...
    print("\n" + "=" * 96)
    print("  LONG VOLATILITY v1 — Hedge Sleeve Research Backtest")
