...
Combines multiple asset × timeframe sleeves of the same strategy into a
fund-style portfolio.  Each sleeve runs on its own allocated capital; the
combined equity is their sum.

Design
------
Asset × timeframe = one sleeve.  All sleeves use the same strategy module
...
def _build_sleeves(args: argparse.Namespace) -> list[SleeveConfig]:
    """Construct active sleeve list from CLI args and weight preset."""
...
def main() -> None:
    ...
    if args.calibrate:
        try:
            ...
            calibrators = {}
            cal = load_calibrator(STRATEGY_NAME, models_dir=args.calibrators_dir)
            if cal is not None and cal.is_fitted:
                calibrators[STRATEGY_NAME] = cal
    ...
    for s in sleeves:
        ...
        log.info(
            "Running sleeve %s — $%.0f (%.0f%%)%s",
            s.label, sleeve_capital, s.weight * 100, cal_tag,
        )
    ...
    # Save portfolio-level equity curve + correlation matrix
