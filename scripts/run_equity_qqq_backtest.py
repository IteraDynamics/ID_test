#!/usr/bin/env python
"""Itera Dynamics — Equity QQQ Trend v1 Backtest Runner.

Runs Equity Sleeve QQQ v1 and prints full research diagnostics.

Outputs:
    artifacts/qqq_trend_v1/
        equity_curve.csv
        trades.csv
        summary.json
"""
...
from research.strategies.equity_qqq_trend_v1 import STRATEGY_ID, generate_intent
...
def main():
    p = argparse.ArgumentParser(description="Run Equity QQQ Trend v1 backtest")
    p.add_argument("--data", required=True)
    p.add_argument("--start", default=None)
    p.add_argument("--end", default=None)
    p.add_argument("--capital", type=float, default=100000)
    p.add_argument("--out-dir", default="artifacts/qqq_trend_v1")
    args = p.parse_args()
    ...
    print("\n" + "=" * 96)
    print("  QQQ DAILY EQUITY TREND v1 — Growth Sleeve Research Backtest")
    ...
    print("  " + "-" * 86)
    print_metrics("Strategy QQQ v1", strategy_m)
    print_metrics("QQQ Buy&Hold", buy_hold_m)
