...
The goal is to identify which parts of the signal survive Coinbase-style
execution friction and which parts are mostly turnover/churn.
...
def _prepare_trades(candidate_dir: Path, cost_case: CostCase, fallback_notional: float, capital: float) -> pd.DataFrame:
    ...
    out["entry_cost"] = out["notional"] * per_side_rate
    out["exit_cost"] = out["notional"] * per_side_rate
...
def _summary_table(df: pd.DataFrame, group_cols: list[str], capital: float) -> pd.DataFrame:
    ...
    for keys, sub in g:
        ...
        trades = int(len(sub))
        target = int((sub["status"] == "target_hit").sum())
        stop = int((sub["status"] == "stop_hit").sum())
...
def _print_top_bottom(title: str, df: pd.DataFrame, label_cols: list[str], top_n: int) -> None:
    ...
    print(f"  {'Group':<44} {'Trades':>7} {'NetPnL':>12} {'Gross':>12} {'Cost':>10} {'Win%':>8} {'AvgNet':>10} {'AvgRet':>8} {'Target':>7} {'Stop':>6} {'Exp':>6}")
    for _, r in view.head(top_n).iterrows():
        print(
            f"  {label(r):<44} {int(r.get('trades') or 0):>7} {_money(r.get('net_pnl')):>12} {_money(r.get('gross_pnl')):>12} {_money(r.get('total_cost')):>10} "
            f"{_fmt(r.get('win_rate_pct')):>8} {_money(r.get('avg_net_pnl')):>10} {_fmt(r.get('avg_net_return_pct')):>8} "
            f"{int(r.get('target_hits') or 0):>7} {int(r.get('stop_hits') or 0):>6} {int(r.get('expired') or 0):>6}"
        )
...
def _print_candidate_rollup(df: pd.DataFrame) -> None:
    roll = _summary_table(df, ["candidate", "cost_case"], capital=100_000.0)
    print("\n" + "=" * DISPLAY_WIDTH)
    print("  COST-ADJUSTED TRADE ATTRIBUTION — CANDIDATE ROLLUP")
    print("=" * DISPLAY_WIDTH)
    if roll.empty:
        print("  No rows.")
        return
    print(f"  {'Candidate':<36} {'CostCase':<12} {'Trades':>7} {'NetPnL':>12} {'Gross':>12} {'Cost':>10} {'Win%':>8} {'AvgNet':>10} {'Target':>7} {'Stop':>6} {'Exp':>6}")
    for _, r in roll.sort_values(["cost_case", "net_pnl"], ascending=[True, False]).iterrows():
        print(
            f"  {str(r.get('candidate')):<36} {str(r.get('cost_case')):<12} {int(r.get('trades') or 0):>7} "
            f"{_money(r.get('net_pnl')):>12} {_money(r.get('gross_pnl')):>12} {_money(r.get('total_cost')):>10} "
            f"{_fmt(r.get('win_rate_pct')):>8} {_money(r.get('avg_net_pnl')):>10} "
            f"{int(r.get('target_hits') or 0):>7} {int(r.get('stop_hits') or 0):>6} {int(r.get('expired') or 0):>6}"
        )


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Analyze cost-adjusted trade attribution for replay candidates")
    p.add_argument(
        "--candidate-dirs",
        nargs="+",
        default=[
            "artifacts/trade_idea_turnover_sweep/looser_stop_12pct__max_new_3",
            "artifacts/trade_idea_turnover_sweep/bucket_cap_60__max_new_3",
        ],
    )
    p.add_argument("--cost-cases", nargs="+", default=["harsh", "very_harsh"], choices=list(DEFAULT_COST_CASES.keys()))
    p.add_argument("--custom-cost-case", nargs="*", default=[], help="Optional cases as name:fee_bps_per_side:slippage_bps_per_side")
...
def main() -> None:
    ...
    summary = {
        ...
        "cost_cases": [c.name for c in cases],
        "capital": args.capital,
        "fallback_notional": args.fallback_notional,
        "realized_trade_rows": int(len(trades_all)),
        "outputs": {name: str(out_dir / f"{name}.csv") for name in dimensions},
    }
    (out_dir / "cost_attribution_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")

    _print_candidate_rollup(trades_all)
    ...
    print("  Verdict             : COST-ADJUSTED ATTRIBUTION ONLY; no broker/runtime execution.\n")
