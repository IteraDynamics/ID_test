#!/usr/bin/env python
"""Itera Dynamics — Short Volatility Sleeve v1 Backtest Runner.

Runs vol_short_v1 against a daily short-vol instrument/proxy such as SVIX or SVXY.

Outputs:
    artifacts/vol_short_v1/
        equity_curve.csv
        trades.csv
        summary.json
"""
...
from research.strategies.vol_short_v1 import STRATEGY_ID, generate_intent
...
def run_backtest(df: pd.DataFrame, capital: float, asset: str):
    exposure = 0.0
    cash = capital
    shares = 0.0
    equity_curve = []
    ...
    for i in range(len(df)):
        ...
        if intent.action == Action.ENTER_LONG and exposure <= 0.01 and target_exposure > 0:
            target_value = target_exposure * nav_before
            delta_shares = target_value / price
            shares += delta_shares
            cash -= target_value
            ...
            trades.append({
                ...
                "delta_notional": target_value,
                "target_exposure": target_exposure,
                "nav_before": nav_before,
                "reason": intent.reason,
            })

        elif intent.action == Action.EXIT_LONG and exposure > 0.01:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Run Short Volatility Sleeve v1 backtest")
    p.add_argument("--data", required=True)
    p.add_argument("--asset", default="SVIX")
    p.add_argument("--start", default=None)
    p.add_argument("--end", default=None)
    p.add_argument("--capital", type=float, default=100000)
    p.add_argument("--out-dir", default="artifacts/vol_short_v1")
    args = p.parse_args()
    ...
    equity, exposure, trades = run_backtest(df, args.capital, args.asset)
    ...
    print("\n" + "=" * 96)
    print("  SHORT VOLATILITY v1 — Carry Sleeve Research Backtest")
    ...
    print_metrics(f"{args.asset} B&H", buy_hold_m)
    ...
    if not trades.empty:
        ...
        for _, row in trades.tail(10).iterrows():
            print(
                ...
                f"notional=${abs(row['delta_notional']):>10,.0f} "
