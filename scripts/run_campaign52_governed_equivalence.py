...
def trade_economics(trades: list[Any]) -> list[dict[str, Any]]:
    fields = (
        "bar_index", "timestamp", "direction", "mid_price", "effective_price", "qty",
        "notional_usd", "fee_usd", "slippage_usd", "spread_usd", "cost_bps",
        "prev_exposure", "new_exposure", "strategy_id",
    )
...
def assert_series_equal(label: str, a: pd.Series, b: pd.Series) -> None:
    if not a.index.equals(b.index) or not np.array_equal(a.to_numpy(), b.to_numpy(), equal_nan=True):
...
def build_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Campaign 52 governed-source capture/replay equivalence only.")
...
def one_pass(args: argparse.Namespace, pass_dir: Path) -> dict[str, Any]:
    ...
    for fold in folds:
        ...
        for spec in specs:
            ...
            canonical = run_backtest(df=df, strategy_module=strategy, initial_capital=spec.capital, exec_config=cfg,
                asset=spec.asset, rebalance_threshold=args.rebalance_threshold, cash_yield_series=cash_yield)
            capture = run_capture_or_replay(df=df, strategy_module=strategy, initial_capital=spec.capital,
                exec_config=cfg, asset=spec.asset, rebalance_threshold=args.rebalance_threshold,
                cash_yield_series=cash_yield, stage=stage, fold=fold.label, sleeve_label=spec.label,
                native_timeframe=spec.timeframe)
            replay = run_capture_or_replay(df=df, strategy_module=None, target_records=capture.targets,
                initial_capital=spec.capital, exec_config=cfg, asset=spec.asset,
                rebalance_threshold=args.rebalance_threshold, cash_yield_series=cash_yield,
                stage=stage, fold=fold.label, sleeve_label=spec.label, native_timeframe=spec.timeframe)

            assert_series_equal(f"canonical_capture_equity:{fold.label}:{spec.label}", canonical.equity_curve, capture.result.equity_curve)
            assert_series_equal(f"canonical_capture_exposure:{fold.label}:{spec.label}", canonical.position_series, capture.result.position_series)
            assert_trade_equal(f"canonical_capture:{fold.label}:{spec.label}", canonical.trades, capture.result.trades)
            assert_series_equal(f"capture_replay_equity:{fold.label}:{spec.label}", capture.result.equity_curve, replay.result.equity_curve)
            assert_series_equal(f"capture_replay_exposure:{fold.label}:{spec.label}", capture.result.position_series, replay.result.position_series)
            assert_trade_equal(f"capture_replay:{fold.label}:{spec.label}", capture.result.trades, replay.result.trades)
            ...
            sleeve_rows.append({"stage": stage, "fold": fold.label, "sleeve": spec.label,
                "target_rows": len(capture.targets), "trade_rows": len(capture.result.trades)})

        canonical_fund = align_equity_curves(canonical_curves, base_freq="1h").sum(axis=1).loc[fold.oos_start:fold.oos_end].dropna()
        capture_fund = align_equity_curves(capture_curves, base_freq="1h").sum(axis=1).loc[fold.oos_start:fold.oos_end].dropna()
        replay_fund = align_equity_curves(replay_curves, base_freq="1h").sum(axis=1).loc[fold.oos_start:fold.oos_end].dropna()
        assert_series_equal(f"fold_fund_canonical_capture:{fold.label}", canonical_fund, capture_fund)
        assert_series_equal(f"fold_fund_capture_replay:{fold.label}", capture_fund, replay_fund)
