#!/usr/bin/env python
"""Build corrected allocator-style finalist reports for trade idea candidates.

This is a corrected finalist report builder. Calendar and monthly returns are
computed from prior period-end equity, not from the current period's first row.
It also avoids pandas.to_markdown so no optional tabulate dependency is required.
...
def _period_returns(adj: pd.DataFrame, label: str, case_name: str, freq: str, period_col: str) -> pd.DataFrame:
    if adj.empty:
        return pd.DataFrame()
    s = adj.set_index("date")["adjusted_equity"].sort_index()
    period_end = s.resample(freq).last().dropna()
    rows = []
    prior_end = None
    for dt, end_eq in period_end.items():
        period_slice = s[(s.index >= dt.to_period(freq[0]).start_time) & (s.index <= dt)] if freq in {"ME", "YE"} else pd.Series(dtype=float)
        if prior_end is None:
            start_eq = float(period_slice.iloc[0]) if not period_slice.empty else float(s.iloc[0])
        else:
            start_eq = float(prior_end)
        ret = (float(end_eq) / start_eq - 1.0) * 100.0 if start_eq else float("nan")
        key = int(dt.year) if period_col == "year" else dt.strftime("%Y-%m")
        rows.append({"candidate": label, "cost_case": case_name, period_col: key, "return_pct": ret, "ending_equity": float(end_eq), "starting_equity": start_eq})
        prior_end = float(end_eq)
    return pd.DataFrame(rows)


def _calendar_year_returns(adj: pd.DataFrame, label: str, case_name: str) -> pd.DataFrame:
    return _period_returns(adj, label, case_name, "YE", "year")


def _monthly_returns(adj: pd.DataFrame, label: str, case_name: str) -> pd.DataFrame:
    return _period_returns(adj, label, case_name, "ME", "month")
...
def _drawdown_periods(adj: pd.DataFrame, label: str, case_name: str, top_n: int) -> pd.DataFrame:
    ...
    return out.sort_values("drawdown_pct").head(top_n) if not out.empty else out
...
def _asset_attribution_by_year(realized: pd.DataFrame, label: str, case: CostCase, fallback_notional: float) -> pd.DataFrame:
    ...
    for _, row in out.iterrows():
        ...
        gross = _to_float(row.get("realized_pnl"), 0.0)
        rows.append({
            "candidate": label,
            "cost_case": case.name,
            "year": int(row["year"]),
    ...
    if detail.empty:
        return detail
    return detail.groupby(["candidate", "cost_case", "year", "asset_class"], as_index=False).agg(
        trades=("net_pnl", "size"),
        gross_pnl=("gross_pnl", "sum"),
        cost=("cost", "sum"),
        net_pnl=("net_pnl", "sum"),
    ).sort_values(["candidate", "year", "asset_class"])
...
def _plain_markdown_table(df: pd.DataFrame, columns: list[str], max_rows: int | None = None) -> str:
    if df.empty:
        return "_No rows._\n"
    ...
    rows = [[str(x) for x in row] for row in display.to_numpy().tolist()]
...
def _write_markdown(out_dir: Path, summary: pd.DataFrame, yearly: pd.DataFrame, rolling: pd.DataFrame, drawdowns: pd.DataFrame, trade_counts: pd.DataFrame, attribution: pd.DataFrame, args: argparse.Namespace) -> None:
    ...
    md.append("## Summary\n")
    md.append(_plain_markdown_table(summary.sort_values(["calmar", "cagr_pct"], ascending=[False, False]), ["candidate", "trades", "cagr_pct", "return_pct", "maxdd_pct", "sharpe", "sortino", "calmar", "total_cost", "net_pnl", "final_equity"]))
    ...
    if not yearly.empty:
        ...
        md.append(_plain_markdown_table(pivot, list(pivot.columns)))
    else:
        md.append("_No rows._\n")
    md.append("## Rolling Return Diagnostics\n")
    md.append(_plain_markdown_table(rolling.sort_values(["window_months", "worst_return_pct"]), ["candidate", "window_months", "worst_return_pct", "median_return_pct", "best_return_pct", "positive_rate_pct", "worst_end_date", "best_end_date"]))
    md.append("## Worst Drawdown Periods\n")
    md.append(_plain_markdown_table(drawdowns, ["candidate", "drawdown_pct", "peak_date", "start_date", "trough_date", "recovery_date", "duration_days"], max_rows=30))
    md.append("## Trade Counts by Year / Status\n")
    md.append(_plain_markdown_table(trade_counts.sort_values(["candidate", "year", "status"]), ["candidate", "year", "status", "trades", "gross_pnl"]))
    md.append("## Asset-Class Attribution by Year\n")
    md.append(_plain_markdown_table(attribution.sort_values(["candidate", "year", "asset_class"]), ["candidate", "year", "asset_class", "trades", "gross_pnl", "cost", "net_pnl"]))
    (out_dir / "finalist_report.md").write_text("\n".join(md), encoding="utf-8")


def _print_summary(df: pd.DataFrame) -> None:
    print("\n" + "=" * DISPLAY_WIDTH)
    print("  TRADE IDEA FINALIST REPORT V2 — CORRECTED PERIOD RETURNS")
    print("=" * DISPLAY_WIDTH)
    if df.empty:
        print("  No rows.")
        return
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build corrected finalist report for trade idea candidate selection")
    ...
    p.add_argument("--out-dir", default="artifacts/trade_idea_candidate_finalist_report_v2")
...
def main() -> None:
    ...
    loaded = [_load_candidate(label, path, case, args) for label, path in finalists.items()]
    ...
    (out_dir / "finalist_report_summary.json").write_text(json.dumps({
        "cost_case": case.name,
    ...
    print("\n  Corrected report written:")
    print(f"    {out_dir / 'finalist_report.md'}")
    print(f"    {out_dir / 'calendar_year_returns.csv'}")
    print(f"    {out_dir / 'monthly_returns.csv'}")
    print("  Verdict: FINALIST REPORT V2 ONLY; corrected period returns; no broker/runtime execution.\n")
