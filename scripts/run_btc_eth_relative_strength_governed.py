...
def _blend_close(btc: pd.Series, eth: pd.Series, btc_weight: float = 0.50) -> pd.Series:
    # Normalize both assets to 1.0 so the trend gate is based on a tradable blend curve,
    # not raw price-scale differences between BTC and ETH.
...
def _write_outputs(
    ...
    (out / "summary.json").write_text(
        json.dumps({"config": vars(args), "results": rows, "exposure_summary": exposures}, indent=2, default=str),
        encoding="utf-8",
    )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--off-exposures", nargs="+", type=float, default=[0.0, 0.25, 0.50])
...
def main() -> None:
    ...
    # Baselines.
    baseline_defs = {
        "btc_bh": _static_weights(common, 1.0),
        "eth_bh": _static_weights(common, 0.0),
    }
    ...
    print("  " + "-" * 154)
    for i, row in enumerate(ranked[:25], start=1):
