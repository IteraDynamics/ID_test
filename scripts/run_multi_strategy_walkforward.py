...
receive explicit BTC macro-state columns before strategy evaluation:
...
def _write_wf_report(out_dir: Path, fold_rows: list[dict], stitched_perf: dict, stitched_annual: dict, audit_rows: list[dict], args: argparse.Namespace) -> None:
    ...
    lines += ["## Stitched OOS Performance", "```text"]
...
def main() -> None:
    ...
    _write_status(out_dir, "stage=loading_data")
    ...
    try:
        ...
        if stitched.empty:
            raise RuntimeError("No stitched OOS NAV produced")
