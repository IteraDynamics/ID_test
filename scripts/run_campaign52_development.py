...
def import_development_targets(root: Path, hashes: Mapping[str, str]) -> dict[tuple[str, str], list[TargetRecord]]:
    imported: dict[tuple[str, str], list[TargetRecord]] = {}
    prefix = "development/"
    target_paths = sorted(k for k in hashes if k.startswith(prefix) and k.endswith("/targets.csv"))
    if len(target_paths) != 27:
        raise Campaign52DevelopmentRunnerError(f"DEVELOPMENT_TARGET_FILE_COUNT:{len(target_paths)}")
    for relative in target_paths:
        parts = relative.split("/")
        if len(parts) != 4 or parts[1] not in DEVELOPMENT_FOLDS:
            raise Campaign52DevelopmentRunnerError(f"INVALID_DEVELOPMENT_TARGET_PATH:{relative}")
...
def replay_ready(records: Sequence[TargetRecord], control_id: str) -> list[TargetRecord]:
    """Normalize non-economic action metadata while preserving transformed targets exactly."""
...
def _secondary_metrics(daily_nav: pd.Series, trades: Sequence[Any]) -> dict[str, float | int]:
    ...
    if current:
        durations.append(current)
    total_cost = float(sum(float(t.fee_usd) + float(t.slippage_usd) + float(t.spread_usd) for t in trades))
    ...
    return {
        ...
        "median_drawdown_recovery_duration_days": float(np.median(durations)) if durations else 0.0,
...
def _bootstrap_comparison(canonical_nav: pd.Series, control_nav: pd.Series, control_id: str) -> dict[str, Any]:
    ...
    if len(can_lr) != len(ctl_lr) or len(can_lr) < 21:
        raise Campaign52DevelopmentRunnerError(f"PAIRED_DAILY_ALIGNMENT_FAILURE:{control_id}")
    n = len(can_lr)
    block = 21
    reps = 10_000
    ...
    indices = (chosen[:, :, None] + np.arange(block)[None, None, :]).reshape(reps, -1)[:, :n]
    ...
    can_dd = 1.0 - np.min(can_paths / np.maximum.accumulate(can_paths, axis=1), axis=1)
    ctl_dd = 1.0 - np.min(ctl_paths / np.maximum.accumulate(ctl_paths, axis=1), axis=1)
    annual_factor = 365.25 / n
    can_ret = np.exp(can_s.sum(axis=1) * annual_factor) - 1.0
    ctl_ret = np.exp(ctl_s.sum(axis=1) * annual_factor) - 1.0
    can_calmar = np.divide(can_ret, can_dd, out=np.full(reps, np.nan), where=can_dd > 0)
    ctl_calmar = np.divide(ctl_ret, ctl_dd, out=np.full(reps, np.nan), where=ctl_dd > 0)
    return {
        ...
        "mean_difference_ci95": [float(np.percentile(paired_means, 2.5)), float(np.percentile(paired_means, 97.5))],
        "one_sided_p": float((np.count_nonzero(paired_means <= 0.0) + 1) / (reps + 1)),
        "drawdown_difference_ci95": [float(np.percentile(ctl_dd - can_dd, 2.5)), float(np.percentile(ctl_dd - can_dd, 97.5))],
        "calmar_difference_ci95": [float(np.nanpercentile(can_calmar - ctl_calmar, 2.5)), float(np.nanpercentile(can_calmar - ctl_calmar, 97.5))],
    }
...
def _one_pass(
    ...
    for cid in CONTROL_IDS:
        ...
        if cid == "static_dev_mean_target":
            for key, records in imported.items():
                transformed[cid][key] = replay_ready(transform_static(records, means), cid)
            transformation_manifest["controls"][cid] = {"type": "static", "means": means}
            ...
            transformation_manifest["controls"][cid] = {"type": "lag", "lag_hours": lag, "streams": counts}
        else:
    ...
    families: dict[str, Mapping[tuple[str, str], Sequence[TargetRecord]]] = {"canonical": imported, **transformed}
    daily_navs: dict[str, pd.Series] = {}
    metrics: dict[str, dict[str, Any]] = {}
    family_trades: dict[str, list[Any]] = {}
    ...
    for family_id, streams in families.items():
        ...
        for fold in folds:
            ...
            fund = align_equity_curves(curves, base_freq="1h").sum(axis=1).loc[fold.oos_start : fold.oos_end].dropna()
            scale = running_nav / float(fund.iloc[0])
            scaled = fund * scale
            running_nav = float(scaled.iloc[-1])
            fold_navs.append(scaled)
            write_series(pass_root / "families" / family_id / fold.label / "fold_fund_nav.csv", scaled, "nav")
        ...
        family_trades[family_id] = all_trades
        metrics[family_id] = {**primary_metrics(daily), **_secondary_metrics(daily, all_trades)}
    ...
    decision = development_decision(metrics["canonical"], {cid: metrics[cid] for cid in CONTROL_IDS}, adjusted)
...
def main() -> None:
    ...
    try:
        canonical_json(temp_root / "input_identity_manifest.json", {
            "campaign": 52,
            "stage": "development",
            "source_sha256": source_hashes,
            "equivalence_manifest_sha256": sha256_file(equivalence_root / "equivalence_manifest.json"),
        ...
        summary = {
            ...
            "development_gate_passed": bool(pass1["decision"]["development_gate_passed"]),
