...
## 🔴 Needs CEO decision
- None from Campaign #58 at this stage — CEO authorized the specification-freeze prerequisites 2026-09-03 (narrow scope; see `ops/decisions.md`). Staff reports the proposed frozen design and power result back for the next governed transition before any real model fit — that report is the next thing headed your way, not a new decision request yet.
...
- **Campaign #58 — specification-freeze prerequisites in progress under 2026-09-03 CEO authorization.** Scope: autocorrelation-corrected power analysis, Phase 0 effective-breadth measurement, candidate grid construction (hard-capped, per Red Team condition 1), fixed hyperparameters per model type, chronological fold/target definitions, causal residualization spec with a provable leakage canary, exact regime-source identification, FDR family definition, and Phase 0 (+ Phase 1 if power-supportable) specification drafting and freeze. No predictor/outcome computation, no model fit, no holdout access. Staff reports the frozen design and power result back before any real model fit is run.
