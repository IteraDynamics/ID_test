...
## 🔴 Needs CEO decision
- **Campaign #58 Phase 1 (time-series residual census): resource a future session to re-test power on the full BTC/ETH/SPY/QQQ/GLD scope, or accept the BTC-only result as a clean FAIL and close the track?** Real, computed result on the one multi-year dataset available this session (Campaign #48's committed BTC anchor inventory, Campaign #53's own governed power methodology reused verbatim): average power at the central IC = 13.0%, against the 50% floor. Per your own conditional ("Phase 1... only if independently supportable by power"), this does not qualify for specification freeze as-is. Whether pooling real ETH/SPY/QQQ/GLD data would raise this enough to matter is unresourced and unknown — this is a resourcing fork, not a routine research call staff should resolve unilaterally. Full record: `docs/research/CAMPAIGN_58_SPECIFICATION_FREEZE_PREREQUISITES_RESULT.md` §10.
...
- **Campaign #58 — specification-freeze prerequisites complete for 2026-09-03 (result reported above/below); durable pieces done regardless of either track's data situation.** Leakage canary built and proven capable of failing on synthetic data (clean pipeline: 0.0064 first-half leak-correlation, no false positive; leaky pipeline: 0.6923, clearly detected). Regime-state source identified and restricted to `research/regimes/baseline_engine.py`'s causal path only, excluding the full-sample discovery tools. Fixed hyperparameters chosen per model type (ridge/elastic net/logistic/linear/shallow RF/shallow GBM, single values not ranges). Fold/target/FDR-family methodology recorded, not yet data-sized. Full record: `docs/research/CAMPAIGN_58_SPECIFICATION_FREEZE_PREREQUISITES_RESULT.md`.
...
## Research queue
1. Campaign #58 (Itera Residual Predictability Census): power analysis run 2026-09-03 — real
   result FAIL (13.0% average power, BTC-only proxy, see 🔴 above). Phase 0 blocked on data/
   network access (see 🟡 above). Next step is the CEO fork above (resource a future
   multi-asset re-test of Phase 1, or close it), and — independent of that — a future session
   with real network/data access to run Phase 0's effective-breadth measurement and power
   analysis on the actual proposed COT universe. No specification is frozen for either track.
