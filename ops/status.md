...
## 🔴 Needs CEO decision
- None from Campaign #58 Phase 1 at this stage — you resolved the prior fork yourself by running the power analysis locally on the full BTC/ETH/SPY/QQQ/GLD dataset. Real result: 58.3% average power at the central IC, PASS. Phase 1 is now eligible to proceed to grid construction and grid-level power testing (the next specification-freeze step, already within your standing authorization — not a new decision). Full record: `docs/research/CAMPAIGN_58_SPECIFICATION_FREEZE_PREREQUISITES_RESULT.md` §11.
...
- **Campaign #58 Phase 1 — base-family power PASS on the real, full multi-asset dataset (58.3% at central IC); next step is sizing and power-testing the actual charter-scoped candidate grid (Red Team condition 1's hard-capped ≤150-candidate family), staff proceeding under standing authorization.** Not uniform across the base family: 5 of 7 candidates individually clear 65-75%, but `drawdown_from_high_trailing_168h` (31.7%) and especially `realized_volatility_trailing_168h` (19.7%) do not — worth carrying into how the full grid is weighted. Durable prerequisite pieces also complete: leakage canary proven capable of failing on synthetic data (clean pipeline: 0.0064 first-half leak-correlation, no false positive; leaky pipeline: 0.6923, clearly detected); regime-state source identified and restricted to `research/regimes/baseline_engine.py`'s causal path only; fixed hyperparameters chosen per model type. Full record: `docs/research/CAMPAIGN_58_SPECIFICATION_FREEZE_PREREQUISITES_RESULT.md`.
...
## Research queue
1. Campaign #58 (Itera Residual Predictability Census): Phase 1 base-family power result is now
   PASS (58.3%, real full multi-asset data, see 🟢 above) — the earlier BTC-only FAIL (13.0%) is
   superseded. Next step: construct and power-test the actual charter-scoped candidate grid
   (Red Team condition 1, hard-capped ≤150), sized against this real result and its
   between-candidate asymmetry. Phase 0 remains blocked on data/network access (see 🟡 above) —
   a future session with real network/data access is needed for its effective-breadth
   measurement and power analysis on the actual proposed COT universe. No specification is
   frozen for either track yet.
