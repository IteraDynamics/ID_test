...
## 🔴 Needs CEO decision
- None from Campaign #58 Phase 1 at this stage. The candidate-grid specification is now frozen and independently Red-Teamed (`CONDITIONAL_PASS`, 10 conditions, all applied). **Real model-fitting is NOT authorized under your standing 2026-09-03 authorization** — that authorization's own text explicitly excludes "fitting real ML models against real predictor/outcome data," so a new decision from you will eventually be needed — but staff is deliberately not asking for it yet. Two cheap prerequisite checks remain first (grid-level power test now scoped to all 3 outcome families, and a leakage-canary re-proof against the real implementation), and staff would rather bring you a decision request once those confirm the frozen grid actually clears power than ask you to authorize execution against an unverified foundation. Full record: `docs/ITERA_CAMPAIGN_BOARD.md` (2026-09-03 same-day correction) and `docs/research/CAMPAIGN_58_PHASE1_FROZEN_STATISTICAL_SPECIFICATION.md`.
...
1. Campaign #58 (Itera Residual Predictability Census): Phase 1's statistical specification is
   frozen and independently Red-Teamed (`CONDITIONAL_PASS`, 10 conditions, applied — see 🟢
   above). Next step: run `scripts/run_campaign58_grid_power_analysis.py` locally against the
   real multi-asset data to verify the actual 144-candidate, 3-outcome-family grid clears 50%
   power (the 58.3% PASS on record was for a 7-candidate, Family-R-only base family, not the
   true frozen grid) — cheap, no new CEO input needed. Real model-fitting remains not authorized
   under the standing 2026-09-03 authorization regardless of that result; staff will bring an
   explicit authorization request only after the grid-level power check and a leakage-canary
   re-proof against the real residualization implementation are both done. Phase 0 remains
   blocked on data/network access (see 🟡 above).
