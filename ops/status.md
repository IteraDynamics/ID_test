...
## 🔴 Needs CEO decision
- **Campaign #58 Phase 1 — grid-level power result is a real FAIL (45.8% at central IC, vs. the 50% floor). Close/deprioritize the time-series track, or authorize an independent (not staff-unilateral) check on a post-hoc methodological question before treating it as final?** Run for real by you locally, trial-adequacy confirmed (min 39 trials/hypothesis, ≥ 20 required) — a trustworthy result. Per family: R 54.9%, M 41.8%, V 40.6%. This independently confirms the Red Team's own concern that a Family-R-only calibration (the tool's first version) would have overstated true grid power — it would have shown 54.9%, a false PASS, 9.1 points above the real blended figure. Used as computed; no design element was adjusted after seeing it. **One honest wrinkle, flagged not acted on:** the calibration tool's residualized-variant columns are numerically identical to their raw counterparts (a disclosed approximation), which staff only now noticed creates 72 exact-duplicate pairs inflating the FDR family size in a way a real run wouldn't reproduce — this might mean the true grid's power is understated by this specific FAIL, or might not; staff is not re-running to check without your sign-off, since doing so unprompted after seeing an inconvenient number is exactly the pattern this campaign's discipline exists to prevent. Full record: `docs/ITERA_CAMPAIGN_BOARD.md` (2026-09-03 same-day correction), `docs/research/CAMPAIGN_58_PHASE1_FROZEN_STATISTICAL_SPECIFICATION.md` §15.
...
1. Campaign #58 (Itera Residual Predictability Census): Phase 1's statistical specification is
   ...
   grid-level power check has now run for real: **FAIL, 45.8%** (see 🔴 above for the result and
   the open CEO fork — close the track, or independently check a post-hoc methodological
   question first). Real model-fitting remains not authorized under the standing 2026-09-03
   authorization regardless. Phase 0 remains blocked on data/network access (see 🟡 above).
