...
- **Screened:** 2026-09-02 under `docs/ITERA_EXPLORATION_SANDBOX.md`; no campaign number.
- **Status:** SCREEN_POSITIVE — PROMOTION RECOMMENDED / CEO CHARTER DECISION REQUIRED.
...
- **Promotion boundary:** per the sandbox protocol, the candidate now returns to the normal governed pipeline. The sandbox sample cannot be reused as untouched confirmation. A genuine independent Red Team review is mandatory before `ALIVE`, and Risk/PM plus CEO approval remain mandatory before any Core v2 composition/weight or capital decision. CIO recommendation: promote to a governed campaign as a potential answer to structural deficiency #2 (single return source), subject to CEO authorization to charter the new direction.
